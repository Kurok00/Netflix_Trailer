  ...
  useEffect(() => {
    ...
    const fetchPageData = async (genresForURL) => {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/discover/${pageName}?api_key=1612773dba4979088f60607d539b18c5&page&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genresForURL}`
      );
    ...
    const fetchSearch = async () => {
      try {
        const { data } = await axios.get(
          `https://api.themoviedb.org/3/search/${pageName}?api_key=1612773dba4979088f60607d539b18c5&page&language=en-US&sort_by=popularity.desc&include_video=false&page=${page}&query=${searchText}&include_adult=false`
        );
        setContents(data.results);
        setNumberOfPages(data.total_pages);
        // console.log(data);
